...
library(ggplot2) 
library(dplyr)
avgdata <- read.csv("\\Users\\jdessy\\Documents\\R\\Shiny Project\\state_average_LC.csv", stringsAsFactors = FALSE)
MI <- read.csv("\\Users\\jdessy\\Documents\\R\\Shiny Project\\MI.csv", header = TRUE)
Ohio <- read.csv("\\Users\\jdessy\\Documents\\R\\Shiny Project\\OH.csv", header = TRUE)
...
ui <- fluidPage(
          ...
          sidebarPanel(
            ...
            selectInput("stateInput", "State",
                             choices = c("Select State","MI", "OH", "MN", "IA", "IN", "IL","MO","ND", "NE","SD","WI")),
            width = 5
